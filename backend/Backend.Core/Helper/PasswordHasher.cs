...
namespace Backend.Core.Helper
{
    public static class PasswordHasher
    {
        ...
        {
            string saltText = $"hu.tanulmanyiversenyek@{email.Split("@")[0]}";

            Rfc2898DeriveBytes hasher = new Rfc2898DeriveBytes(saltText, Encoding.Default.GetBytes(saltText), 10000, HashAlgorithmName.SHA512);
        ...
        {
            Rfc2898DeriveBytes Hasher = new Rfc2898DeriveBytes(password, Encoding.Default.GetBytes(salt), 10000, HashAlgorithmName.SHA512);
            string result = Convert.ToBase64String(Hasher.GetBytes(25));
